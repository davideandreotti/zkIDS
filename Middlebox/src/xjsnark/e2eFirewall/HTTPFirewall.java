...
public class HTTPFirewall {
  ...
  public static final int[] http11_ints = {0x48, 0x54, 0x54, 0x50, 0x2f, 0x32, 0x2e, 0x30};
  // MODIFIED: now is HTTP/2.0
  ...
  private static UnsignedInteger get_first_crlf_index(UnsignedInteger[] http_msg) {
    ...
    for (int i = 1; i < HTTP_REQUEST_MAX_LENGTH; i++) {
      ...
      {
        Bit bit_j0o0s = CRLF_CONCAT.isEqualTo(curr_concat).copy();
        boolean c_j0o0s = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_j0o0s);
        if (c_j0o0s) {
          if (bit_j0o0s.getConstantValue()) {
          ...
          ConditionalScopeTracker.push(bit_j0o0s);
