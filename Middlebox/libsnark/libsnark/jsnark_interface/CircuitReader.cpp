...
CircuitReader::CircuitReader(char* arithFilepath, ProtoboardPtr pb) {
	cout << "No inputs provided: generating circuit only" << endl;
	this->pb = pb;
	numWires = 0;
	numInputs = numNizkInputs = numOutputs = 0;
...
void CircuitReader::parseOnly(char* arithFilepath){
	libff::enter_block("Parsing Only the circuit");
	...
	while (getline(arithfs, line)) {
		...
		if (line[0] == '#') {
			...
			while (iss_i >> inWireId) {
				...
				inValues.push_back(wireValues[inWireId]); //needed?
			}
	...
	if (!inputfs.good()) {
		...
		while (getline(inputfs, line)) {
			...
			if (2 == sscanf(line.c_str(), "%u %s", &wireId, inputStr)) {
				wireValues[wireId] = readFieldElementFromHex(inputStr);	//assigns the value to the input wires!
			} else {
...
void CircuitReader::constructCircuit(char* arithFilepath) {
	...
	unsigned int i;
	//cout << numInputs <<" "<< numOutputs <<" "<< numNizkInputs << endl;
...
//		}
		...
		if (5 == sscanf(line.c_str(), "%s in %d <%[^>]> out %d <%[^>]>", type, &numGateInputs, inputStr, &numGateOutputs, outputStr)) {
			if (strcmp(type, "add") == 0) {
...
void CircuitReader::mapValuesToProtoboard() {
	...
	for (WireMap::iterator iter = variableMap.begin();
		...
		pb->val(*variables[variableMap[wireId]]) = wireValues[wireId]; //Check if important
