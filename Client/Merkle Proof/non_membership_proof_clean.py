...
def runMembershipProof(command, leaf, merkle_tree_structure, input_array):
	root = merkle_tree_structure[-1]
	height = merkle_tree_height(len(input_array))
	index = input_array.index(leaf)
...
print("Computing metkle tree structure")
...
# TODO: if read merkle tree from file: convert to int!
'''
	length = len(input_array)
	start = time.time()
	for j in range(0, length):
...
real_leaf = find_hidden_substring(command, hashed_array)
print(real_leaf)
print("Running membership proof")
return_value = runMembershipProof(command, real_leaf, merkle_tree_structure, input_array)
print(return_value)
