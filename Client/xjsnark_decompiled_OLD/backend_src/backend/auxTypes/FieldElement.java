...
public class FieldElement implements IAuxType, ConditionalScopeImpactedType {
   ...
   public void pop(int id) {
      if (this.stateChanged) {
         ...
         --this.scope;
         FieldElement prev = (FieldElement)this.prevValStack.peek();
   ...
   public void popMain() {
      if (this.generator.__getPhase() == 0) {
         ...
         HashMap<Integer, FieldElement> possibleVals = (HashMap)this.possibleValStack.pop();
         int mulIndex = ((FieldElement)this.prevValStack.pop()).getVariableState().getMulIndex();
         Iterator var4 = possibleVals.keySet().iterator();

         while(var4.hasNext()) {
            Integer idx = (Integer)var4.next();
            int m = ((FieldElement)possibleVals.get(idx)).variableState.getMulIndex();
            if (m > mulIndex) {
               mulIndex = m;
            }
         }
         ...
         FieldElement[] candidateList = new FieldElement[numberOfValues];
         HashMap<Integer, FieldElement> possibleVals = (HashMap)this.possibleValStack.pop();

         Integer idx;
         for(Iterator var8 = possibleVals.keySet().iterator(); var8.hasNext(); candidateList[idx] = (FieldElement)possibleVals.get(idx)) {
         ...
         FieldElement initial = candidateList[numberOfValues - 1];
         int startingIndex = -1;
         ...
         if (startingIndex == -1) {
            ...
            for(int j = startingIndex; j >= 0; --j) {
               FieldElement current = candidateList[j];
   ...
   public FieldElement copy() {
      FieldElement v;
      if (this.generator.__getPhase() == 0) {
         v = new FieldElement(this.generator, this.modulus, this.variableState);
   ...
   public FieldElement add(BigInteger b) {
      return this.add(new FieldElement(this.modulus, b));
   }

   public FieldElement mul(BigInteger b) {
      return this.mul(new FieldElement(this.modulus, b));
   }

   public FieldElement add(FieldElement o) {
      if (this.isConstant() && o.isConstant()) {
         return new FieldElement(this.modulus, this.getConstant().add(o.getConstant()));
         ...
         FieldElement result = new FieldElement(this.generator, this.modulus);
         ...
         FieldElement result = new FieldElement(this.modulus, this.packedWire.add(o.packedWire), outMaxValue);
   ...
   public FieldElement mul(FieldElement o) {
      if (this.isConstant() && o.isConstant()) {
         return new FieldElement(this.modulus, this.getConstant().multiply(o.getConstant()));
         ...
         return new FieldElement(this.modulus, BigInteger.ZERO);
      } else if (o.isConstant() && o.getConstant().equals(BigInteger.ZERO)) {
         return new FieldElement(this.modulus, BigInteger.ZERO);
      } else if (this.generator.__getPhase() == 0) {
         this.variableState.setPackedAhead(true);
         ...
         PackedValue.disableOverflowChecks = true;
         FieldElement result = new FieldElement(this.modulus, this.packedWire.mul(o.packedWire), outMaxValue);
         PackedValue.disableOverflowChecks = false;
         return result;
      }
   }
   ...
   public FieldElement subtract(BigInteger b) {
      return this.subtract(new FieldElement(this.modulus, b));
   }

   public FieldElement subtract(FieldElement o) {
      if (this.isConstant() && o.isConstant()) {
         return new FieldElement(this.modulus, this.getConstant().subtract(o.getConstant()));
      } else if (o.isConstant() && o.getConstant().equals(BigInteger.ZERO)) {
         CircuitGenerator.__getActiveCircuitGenerator().__retrieveVariableState();
         return this.copy();
      } else if (this.generator.__getPhase() == 0) {
         ...
         o.variableState.decAddUseCount();
         if (this.nativeSnarkField) {
            return new FieldElement(this.modulus, new PackedValue(this.packedWire.array[0].sub(o.packedWire.array[0]), this.modulus.subtract(BigInteger.ONE)), this.modulus.subtract(BigInteger.ONE));
         } else {
            BigInteger[] aux = SubtractionAUX.prepSub(o.packedWire, this.modulus, this.generator, this.modulus.bitLength());
            boolean overflowCheck = this.packedWire.addSubOverflowCheck(aux);
            if (overflowCheck) {
               handleOverflow(this, o, false);
               aux = SubtractionAUX.prepSub(o.packedWire, this.modulus, this.generator, this.modulus.bitLength());
            ...
            BigInteger outMaxValue = this.maxValue.add(Util.group(aux, UnsignedInteger.BITWIDTH_PER_CHUNK));
            BigInteger[] a = SubtractionAUX.prepSub(o.packedWire, this.modulus, this.generator, this.modulus.bitLength());
            FieldElement result = new FieldElement(this.modulus, this.packedWire.addsub(a, o.packedWire), outMaxValue);
            return result;
         }
      }
   }
   ...
   public FieldElement negate() {
      return this.isConstant() ? new FieldElement(this.modulus, this.modulus.subtract(this.getConstant())) : (new FieldElement(this.modulus, BigInteger.ZERO)).subtract(this);
   }
   ...
   public FieldElement div(FieldElement b) {
      ...
      if (this.isConstant() && b.isConstant()) {
         ...
         return new FieldElement(this.modulus, aux);
      } else if (this.generator.__getPhase() == 0) {
         this.variableState.setPackedAhead(true);
         int idx1 = this.getVariableState().getMulIndex();
         this.variableState.setConditionallySplittedAhead(true);
         ...
         if (this.nativeSnarkField) {
            ...
            PackedValue.disableOverflowChecks = false;
            return new FieldElement(this.modulus, new PackedValue(f.getOutputWires()[0], this.modulus.subtract(BigInteger.ONE)), this.modulus.subtract(BigInteger.ONE));
         } else {
            ...
            if (this.modulus.bitLength() > UnsignedInteger.BITWIDTH_LIMIT_SHORT) {
               ...
               result = new FieldElement(this.modulus, v, Util.computeMaxValue(this.modulus.bitLength()));
               return result;
            } else {
   ...
   public FieldElement inv() {
      if (this.isConstant()) {
         return new FieldElement(this.modulus, this.getConstant().modInverse(this.modulus));
      } else if (this.generator.__getPhase() == 0) {
         if (Config.enforceInternalDivisionNonZeroChecks) {
   ...
   public static FieldElement createInput(CircuitGenerator generator, BigInteger modulus, String... desc) {
      ...
      FieldElement o = new FieldElement(modulus, v, modulus.subtract(BigInteger.ONE));
   ...
   public static FieldElement[] createZeroArray(CircuitGenerator generator, int size, BigInteger modulus, String... desc) {
      FieldElement[] out = new FieldElement[size];

      for(int i = 0; i < size; ++i) {
         out[i] = new FieldElement(modulus, BigInteger.ZERO);
      }
   ...
   public static Object createZeroArray(CircuitGenerator generator, int[] dims, BigInteger modulus, String... desc) {
      if (dims.length == 1) {
         ...
         int i;
         if (dims.length == 2) {
            FieldElement[][] out = new FieldElement[dims[0]][];
            ...
            FieldElement[][][] out = new FieldElement[dims[0]][dims[1]][];
   ...
   public static void makeOutput(CircuitGenerator generator, Object a, String... desc) {
      ...
      if (a instanceof FieldElement[]) {
         FieldElement[] array = (FieldElement[])a;

         for(i = 0; i < array.length; ++i) {
            makeOutput(generator, array[i], desc);
         }
      } else if (a instanceof FieldElement[][]) {
         FieldElement[][] array = (FieldElement[][])a;
   ...
   public FieldElement mul(Bit bit) {
      return this.mul(new FieldElement(this.modulus, new PackedValue(bit.wire, BigInteger.ONE), BigInteger.ONE));
   }

   public FieldElement add(Bit bit) {
      return this.add(new FieldElement(this.modulus, new PackedValue(bit.wire, BigInteger.ONE), BigInteger.ONE));
   }
   ...
   public void forceEqual(IAuxType o) {
      if (!(o instanceof FieldElement)) {
         ...
         FieldElement other = (FieldElement)o;
   ...
   public static FieldElement instantiateFrom(BigInteger modulus, int v) {
      return new FieldElement(modulus, BigInteger.valueOf((long)v));
   }

   public static FieldElement instantiateFrom(BigInteger modulus, long v) {
      return new FieldElement(modulus, BigInteger.valueOf(v));
   }

   public static FieldElement instantiateFrom(BigInteger modulus, BigInteger v) {
      return new FieldElement(modulus, v);
   }
   ...
   public static FieldElement instantiateFrom(BigInteger modulus, UnsignedInteger e) {
      if (e.isConstant()) {
         return new FieldElement(modulus, e.getConstant());
      } else {
         ...
         if (generator.__getPhase() == 0) {
            ...
            result = new FieldElement(generator, modulus);
            return result;
            ...
            if (e.maxValue.compareTo(modulus) >= 0 && e.maxValue.bitLength() <= UnsignedInteger.BITWIDTH_LIMIT_SHORT) {
               ...
               FieldElement result = new FieldElement(modulus, new PackedValue(modCGadget.getOutputWires()[0], modulus.subtract(BigInteger.ONE)));
               return result;
               ...
               FieldElement result = new FieldElement(modulus, new PackedValue(g.getRemainder().array, maxVals), modulus.subtract(BigInteger.ONE));
               return result;
            } else {
               result = new FieldElement(modulus, e.packedWire, e.maxValue);
               return result;
            }
         } else {
            ...
            FieldElement result = new FieldElement(modulus, new PackedValue(w, maxVal), maxVal);
            return result;
         }
      }
   }
   ...
   public static FieldElement instantiateFrom(BigInteger modulus, String v) {
      return new FieldElement(modulus, new BigInteger(v));
   }

   public static FieldElement[] instantiateFrom(BigInteger modulus, int[] v) {
      FieldElement[] a = new FieldElement[v.length];
