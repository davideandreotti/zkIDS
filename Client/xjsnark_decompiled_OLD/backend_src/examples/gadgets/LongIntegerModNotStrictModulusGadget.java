...
public class LongIntegerModNotStrictModulusGadget extends Gadget {
   ...
   private void buildCircuit() {
      ...
      int rChunkLength = (int)Math.ceil((double)rBitwidth * 1.0 / (double)UnsignedInteger.BITWIDTH_PER_CHUNK);
      int qChunkLength = (int)Math.ceil((double)aBitwidth * 1.0 / (double)UnsignedInteger.BITWIDTH_PER_CHUNK);
      ...
      if (rBitwidth % UnsignedInteger.BITWIDTH_PER_CHUNK != 0) {
         rChunkBitwidths[rChunkLength - 1] = rBitwidth % UnsignedInteger.BITWIDTH_PER_CHUNK;
      }

      if (aBitwidth % UnsignedInteger.BITWIDTH_PER_CHUNK != 0) {
         qChunkBitwidths[qChunkLength - 1] = aBitwidth % UnsignedInteger.BITWIDTH_PER_CHUNK;
      }
      ...
      this.generator.__specifyProverWitnessComputation(new Instruction() {
         public void evaluate(CircuitEvaluator evaluator) {
            BigInteger aValue = evaluator.getWireValue(LongIntegerModNotStrictModulusGadget.this.a, UnsignedInteger.BITWIDTH_PER_CHUNK);
            BigInteger bValue = evaluator.getWireValue(LongIntegerModNotStrictModulusGadget.this.b, UnsignedInteger.BITWIDTH_PER_CHUNK);
            BigInteger rValue = aValue.mod(bValue);
            BigInteger qValue = aValue.divide(bValue);
            evaluator.setWireValue(LongIntegerModNotStrictModulusGadget.this.r.getArray(), Util.split(rValue, UnsignedInteger.BITWIDTH_PER_CHUNK));
            evaluator.setWireValue(LongIntegerModNotStrictModulusGadget.this.q.getArray(), Util.split(qValue, UnsignedInteger.BITWIDTH_PER_CHUNK));
         }
      });
      ...
      if (this.restrictRange) {
         this.generator.__addOneAssertion(this.r.isLessThan(this.b, rBitwidth).getWire());
      }
