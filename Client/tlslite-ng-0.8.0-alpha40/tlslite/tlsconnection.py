...
class TLSConnection(TLSRecordLayer):
    ...
    def __init__(self, sock):
        ...
        # zombie 
        ## EC curve points
        self.clientPublic = bytearray(0)
        self.clientPrivate = ""
        self.serverPublic = bytearray(0)

        ## H_2 for SHTS 
        self.serverHelloTranscript = bytearray(0)
        self.serverHelloTranscriptHash = bytearray(0)

        ## TR_3 for CATS
        self.serverFinishedTranscript = bytearray(0)
        self.serverFinishedTranscriptHash = bytearray(0)

        #ZOMBIE Separate Record Layers for ServerHello Transcript
        self.handshakeEncryptedExtensionsRecordLayer = bytearray(0)
        self.handshakeCertificateRecordLayer = bytearray(0)
        self.handshakeCertificateVerifyRecordLayer = bytearray(0)
        self.handshakeFinishedRecordLayer = bytearray(0)
        
        ## Server Handshake Keys
        self.serverHSKey = bytearray(0)
        self.serverHSIV = bytearray(0)

        # resumption flag for padding
        self.method_flag_for_padding = "dot"
        self.is_resumption_for_padding = False
        
        ## client application keys
        # use ._recordLayer._writeContext.encContext.key for key
        # use ._recordLayer._writeContext.fixedNonce for iv

        ## DNS message 
        # use ._recordLayer.plaintextMessage
        # use ._recordLayer.encryptedMessage[0:-34]
    ...
    def handshakeClientCert(self, certChain=None, privateKey=None,
                            ...
                            async_=False, alpn=None, print_handshake=False):
        ...
        """
        if print_handshake:
            self.printHandshake = print_handshake
        handshaker = \
    ...
    def _clientSendClientHello(self, settings, session, srpUsername,
        ...
        if next((i for i in settings.versions if i > (3, 3)), None):
            ...
            if self.printHandshake:
                session_id = bytes.fromhex('28f7439c5b3a57e898442bc482ae09b309b2d79aac5e542768cf0c11b0c393f2')
                print("Using hard-coded session id "+session_id.hex())
            ...
            for group_name in settings.keyShares:
                ...
                # zombie (need to be careful with multiple key shares?)
                self.clientPublic = key_share.key_exchange.hex()
                self.clientPrivate = hex(key_share.private)

                if self.printHandshake and self.fixedCurvePoint:
                    our_key_share = KeyShareEntry()
                    our_key_share.group = key_share.group
                    our_key_share.key_exchange = bytearray(bytes.fromhex('94077f6fee4f97c20c336e4f13416c869841db937b1da9cc27219ebce5a5d905'))
                    our_key_share.private = bytearray(bytes.fromhex('f86cc2ff06097aea40a3f639fc35752a567a835db7c134d0d1011b28c0121e61'))
                    key_share = our_key_share
                    print("Using hard-coded DH value!")
                    print("key share group: "+str(key_share.group))
                    print("key share public: "+str(key_share.key_exchange.hex()))
                    print("key share private: "+str(key_share.private.hex()))
        ...
        client_random = getRandomBytes(32)
        self._clientRandomFake = client_random
        if self.printHandshake:
        #    client_random = bytearray(bytes.fromhex('441C0100000200000000000006676F6F676C6503636F6D0000010001C00C00010001'[:64]))
            print("Using client random: " + client_random.hex())
        ...
        if session and session.sessionID:
            ...
            if session.cipherSuite not in cipherSuites:
                ...
                clientHello.create(sent_version, client_random,
        ...
        if settings.usePaddingExtension:
            HandshakeHelpers.alignClientHelloPadding(clientHello, self.method_flag_for_padding, self.is_resumption_for_padding)
    ...
    def _clientTLS13Handshake(self, settings, session, clientHello,
        ...
        self.messages.append(copy.deepcopy(clientHello))
        self.messages.append(copy.deepcopy(serverHello))
        if self.printHandshake:
            print("Appending clientHello object to handshake transcript.")
            print(clientHello)
            print("\n")
            print("Appending serverHello object to handshake transcript.")
            print(serverHello)
            print("")
        ...
        if sr_kex:
            ...
            # zombie
            self.serverPublic = sr_kex.key_exchange.hex()
        ...
        # zombie
        self.handshakeSecret = secret.hex()
        ...
        # zombie
        self.serverHelloTranscript = self._handshake_hash._text.hex()

        self.serverHelloTranscriptHash = self._handshake_hash.digest('sha256').hex()

        if self.printHandshake:
            #print("client handshake traffic secret: " + cl_handshake_traffic_secret.hex())
            #print("server handshake traffic secret: " + sr_handshake_traffic_secret.hex())
            with open('dummy_pms_log.txt', 'a') as keylogfile:
                keylogfile.write("CLIENT_HANDSHAKE_TRAFFIC_SECRET " + self._clientRandomFake.hex() + " " + cl_handshake_traffic_secret.hex()+"\n")
                keylogfile.write("SERVER_HANDSHAKE_TRAFFIC_SECRET " + self._clientRandomFake.hex() + " " + sr_handshake_traffic_secret.hex()+"\n")
        ...
        ## zombie 
        self.serverHSKey = self._recordLayer._readState.encContext.key
        self.serverHSIV = self._recordLayer._readState.fixedNonce
        ...
        #zombie
        self.handshakeEncryptedExtensionsRecordLayer = encrypted_extensions.write().hex()

        self.messages.append(copy.deepcopy(encrypted_extensions))
        if self.printHandshake:
            print("Appended EncryptedExtensions object to handshake transcript.")
            print(encrypted_extensions.getExtension(ExtensionType.post_handshake_auth))
        assert isinstance(encrypted_extensions, EncryptedExtensions)

        size_limit_ext = encrypted_extensions.getExtension(
            ExtensionType.record_size_limit)
        if self.printHandshake:
            print("printing record_size_limit extension")
            print(size_limit_ext)
        ...
        if not sr_psk:
            ...
            certificate = result
            #zombie
            self.handshakeCertificateRecordLayer = certificate.write().hex()

            if self.printHandshake:
                print("printing certificate extension")
                print(certificate)
            ...
            self.handshakeCertificateVerifyRecordLayer = certificate_verify.write().hex()
        ...
        ## zombie : this is the H_7 hash value to compute Server Finished value
        server_extensions_hs = self._handshake_hash.copy()
        self.serverExtensionsTranscript = server_extensions_hs._text.hex()
        self.serverExtensionsTranscriptHash = server_extensions_hs.digest('sha256').hex()
        ...
        for result in self._getMsg(ContentType.handshake,
            ...
            if result in (0, 1):
                ...
                break
        finished = result
        self.handshakeFinishedRecordLayer = finished.write().hex()

        ## zombie : this should be the Server Finished value
        self.serverFinishedValue = finished.verify_data.hex()

        server_finish_hs = self._handshake_hash.copy()

        ## zombie : this is the H_3 hash value to compute CATS
        self.serverFinishedTranscript = server_finish_hs._text.hex()
        self.serverFinishedTranscriptHash = server_finish_hs.digest('sha256').hex()
        ...
        sr_app_traffic = derive_secret(secret, bytearray(b's ap traffic'),
                                       server_finish_hs, prfName)
        if self.printHandshake:
            print("client app traffic secret: " + cl_app_traffic.hex())
            print("server app traffic secret: " + sr_app_traffic.hex())
            print("client random self: " + self._clientRandom.hex())
            with open('dummy_pms_log.txt', 'a') as keylogfile:
                keylogfile.write("CLIENT_TRAFFIC_SECRET_0 " + self._clientRandomFake.hex() + " " + cl_app_traffic.hex()+"\n")
                keylogfile.write("SERVER_TRAFFIC_SECRET_0 " + self._clientRandomFake.hex() + " " + sr_app_traffic.hex()+"\n")
        ...
        # Finished
        self._changeReadState() # zombie: get server's app keys from here
    ...
    def _serverTLS13Handshake(self, settings, clientHello, cipherSuite,
        ...
        print("CertVerify", cli_cert_verify_hh)
